...
DATA_LIST_PATH='./dataset/city/train_aug_ic19.txt'
OUTPUT_ROOT='/home/garbade/models_tf/05_Cityscapes/26_predNet_ganLoss_ic19/'
#RESTORE_FROM = './deeplab_tf_model/deeplab_resnet_init.ckpt'
RESTORE_FROM = '/home/garbade/models_tf/05_Cityscapes/22_msc_fullSizeInput_nc19/snapshots_finetune/model.ckpt-20000'
...
#DATA_LIST_PATH = './dataset/voc12/train_Bndry255.txt'
#OUTPUT_ROOT='/home/garbade/models_tf/01_voc12/22_nc21_fullResolution_Bndry255/'
#RESTORE_FROM = './deeplab_tf_model/deeplab_resnet_init.ckpt'
...
def get_arguments():
    ...
    parser.add_argument("--restoring_mode", type=str, default=RESTORING_MODE,
                        help="Specify restoring_mode.")
    ...
    parser.add_argument("--ignore_label", type=int, default=ignore_label,
                        help="Ignore label class number.")   
    parser.add_argument("--loss_weight", type=float, default=loss_weight,
                        help="loss_weight.")                           
    parser.add_argument("--dnet_dir", type=str, default=dnet_dir,
                        help="Directory containing d_net weights.")
...
def main():
    ...
    # Output dirs
    SAVE_DIR = args.output_root + '/images_finetune/'
    SNAPSHOT_DIR = args.output_root + '/snapshots_finetune/'
    ...
    with tf.variable_scope('', reuse=True):
        net075 = DeepLabResNetModel({'data': image_batch075},args.n_classes, is_training=args.is_training)
    with tf.variable_scope('', reuse=True):
        net05 = DeepLabResNetModel({'data': image_batch05},args.n_classes, is_training=args.is_training)
    ...
    if USE_GAN_LOSS:
        print('GAN Loss is switched on:')
	# Discriminator loss
        d_net_input = tf.argmax(raw_output, dimension=3)
        d_net_input = tf.expand_dims(d_net_input,axis=3)    
        d_net_input = tf.cast(d_net_input, tf.float32)    
        d_net = DCGAN(d_net_input)
        d_net_loss = d_net.get_loss()    
        reduced_loss = tf.reduce_mean(loss) + tf.reduce_mean(loss100) + tf.reduce_mean(loss075) + tf.add_n(l2_losses) + args.loss_weight * d_net_loss
    ...
    if args.restore_from is not None:
        if args.restoring_mode == 'restore_all':
            print('Restoring all layers. (Restoring from pretrained net)')
    ...
    for step in range(args.num_steps):
        ...
        if step % args.save_pred_every == 0:
            ...
            plt.savefig(SAVE_DIR + str(start_time) + str(step) + ".png")
            plt.close(fig)
        else:
